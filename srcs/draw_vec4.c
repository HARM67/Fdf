...
/*   Updated: 2016/01/30 16:14:16 by mfroehly         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

static void	draw_vec4_no_prof(t_app *app, t_vec4 tmp)
{
...
static void	draw_vec4_prof(t_app *app, t_vec4 tmp)
{
	app->data[(int)(tmp.y + 0.5) * 4 * WIDTH + (int)(tmp.x + 0.5) * 4] =
		tmp.color.b * (1 - tmp.z);
	app->data[(int)(tmp.y + 0.5) * 4 * WIDTH + (int)(tmp.x + 0.5) * 4 + 1] =
...
void		draw_vec4(t_app *app, t_vec4 tmp, t_obj *o)
{
	...
	app->z_buffer[(int)((int)(tmp.x + 0.5) + ((int)(tmp.y + 0.5) * WIDTH))] =
		tmp.z;
	if (app->a_prof)
		draw_vec4_prof(app, tmp);
	else
		draw_vec4_no_prof(app, tmp);
	if (!app->a_picking)
		return ;
